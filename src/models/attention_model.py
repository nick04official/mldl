...
class NewAttentionModel(nn.Module):
    ...
    def __init__(self, num_classes=61, mem_size=512, no_cam=False, enable_motion_segmentation=False):
        ...
        self.ccam = Variable((torch.FloatTensor(512).normal_(0, .05)).unsqueeze(0).cuda())
    ...
    def get_training_parameters(self, name='all'):
        ...
        if self._custom_train_mode != False:
            for params in self.classifier.parameters():
                params.requires_grad = True
                train_params += [params]
            self.ccam.requires_grad = True
            train_params += [self.ccam]
    ...
    def forward(self, inputVariable):
        ...
        for t in range(inputVariable.size(0)):
            _, feature_conv, feature_convNBN = self.resnet(inputVariable[t])

            bz, nc, h, w = feature_conv.size()
            feature_conv1 = feature_conv.view(bz, nc, h*w)
            
            cam = torch.bmm(self.ccam[[0] * inputVariable.size(1)].unsqueeze(1), feature_conv1)
            attentionMAP = F.softmax(cam.squeeze(1), dim=1)
            attentionMAP = attentionMAP.view(attentionMAP.size(0), 1, 7, 7)
            attentionFeat = feature_convNBN * attentionMAP.expand_as(feature_conv)
...
class NewAttentionModelBi(nn.Module):
    ...
    def __init__(self, num_classes=61, mem_size=512, no_cam=False):
        ...
        self.attention_rgb = Variable((torch.FloatTensor(512).normal_(0, .05)).unsqueeze(0).cuda())
    ...
    def get_resnet_output_feats(self, resnet, attention, input_frames):
        ...
        cam = torch.bmm(attention[[0] * input_frames.size(0)].unsqueeze(1), feature_conv1)
        attentionMAP = F.softmax(cam.squeeze(1), dim=1)
        attentionMAP = attentionMAP.view(attentionMAP.size(0), 1, 7, 7)
        attentionFeat = feature_convNBN * attentionMAP.expand_as(feature_conv)
    ...
    def forward(self, rgb_frames, flow_frames):
        state = (Variable(torch.zeros((rgb_frames.size(1), self.mem_size, 7, 7)).cuda()),
                 Variable(torch.zeros((rgb_frames.size(1), self.mem_size, 7, 7)).cuda()))
                
        for t in range(rgb_frames.size(0)):
            rgb_feats = self.get_resnet_output_feats(self.resnet, self.attention_rgb, rgb_frames[t])
            flow_feats = self.get_resnet_output_feats(self.resnet, self.attention_flow, flow_frames[t])
            state = self.lstm_cell(torch.cat((rgb_feats, flow_feats), dim=1), state)
