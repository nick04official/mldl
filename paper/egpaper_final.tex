...
We also propose a two stream variant, which consists in a shared backbone, fed with both colorised warp flow frames and original RGB frames, and whose output features are fed to two ConvLSTMs.
...
By using a single ConvLSTM cell acting simultaneously on ``RGB features'' and ``flow features'', besides modelling temporal dependencies of appearance and motion, we believe that correlations between motion and appearance can also be modelled as well.
...
We trained the RGB network following the same protocol described in \ref{par:Ego-RNN} and using the same hyper-parameters. However, we used a lower number of frames: we sampled $N = 7$ and $N = 16$ frames uniformly spaced in time.
