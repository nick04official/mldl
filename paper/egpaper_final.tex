...
To summarise, we can think of a CAM as a linear classifier: given a spatial location, it outputs a score which states the relevance of that spatial location regarding the performed action; by applying the softmax operation on the scores for all the spatial positions of the input, these scores can be turned into normalised scores (or probabilities) regarding how that position is relevant for the action; then, by dot multiplying the normalised scores and input, the interesting regions of the input are amplified. The main reasoning behind this procedure is that different actions cause different neurons in the hidden fully connected layer to activate; therefore, by computing the CAM using the weights of the neuron of the winning class, action-specific regions of the input frame are highlighted. However, we verified that this is not the case: by replicating the experiments proposed in \cite{Ego-RNN}, and also by running their trained model published online, we verified that, at convergence, the same hidden neuron is activated for all the input samples.
...
Our proposed method draws itss. Our method extracts RGB frames from warp flow frames: by leveraging a pre-existing RGB network which features a ResNet-34 backbone already trained on ImageNet, our method aims to replicate on flow frames the results achieved by this architecture on standard RGB frames.
...
\begin{figure*}
	\begin{center}
		\includegraphics[width=\textwidth]{schemi/two_stream1_img.pdf}		
	\end{center}
	\caption{Two-stream RGB network}
	\label{fig:TwoStream}
\end{figure*}

We believe that, in order to perform the action recognition task in the first person point of view, the warp flow may not be sufficient  by itself to achieve high performances, but the appearances of the objects may play an important role as well. In fact, no special equipment is used at collection time to obtain the warped optical flow: computing the warped optical flow can be seen as projecting the original data in another space where movement is highlighted and appearance is discarded. Therefore we argue that the warp flow helps deep learning models in encoding motion in an effectively way, but at the same time, the appearance of the object is useful data that would be discarded if only warp flow frames were employed. For this reason, we propose a two-stream network featuring a shared backbone.

This architecture is represented in figure \ref{fig:TwoStream} and is very similar to the previous variant: it features a shared backbone pre-trained on ImageNet and it is fed with both RGB and warp flow frames. This is possible because the warp flow frames are actually processed first by WFCNet, which is used to infer RGB images starting from warp flow frames. An attention mechanism is also employed on the output features of the backbone as explained in paragraph \ref{par:AttentionModel}.

Two separate ConvLSTM cells are employed: one cell operates on the feature extracted by the backbone from the RGB frames, the other on the feature extracted from the warp flow frames. Then, after being reduced by an average pooling layer, the output features of the two ConvLSTM cells are concatenated, and a fully connected layer is employed to compute the class scores.
...
\begin{itemize}
	...
	\item a Flow Network, which consists in a ResNet backbone pre-trained on ImageNet
\end{itemize}
