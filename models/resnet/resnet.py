...
class ResNet(nn.Module):
    ...
    def train(self, mode=True):
        ...
        if self._custom_train_mode == 'stage2':
            for i in [0, 1, 2]:
                self.layer4[i].conv1.train(True)
                self.layer4[i].conv2.train(True)
            self.fc.train(True)
    ...
    def get_training_parameters(self):
        ...
        if self._custom_train_mode == 'stage2':
            for i in [0, 1, 2]:
                for params in self.layer4[i].conv1.parameters():
                    params.requires_grad = True
                    train_params += [params]

                for params in self.layer4[i].conv2.parameters():
                    params.requires_grad = True
                    train_params += [params]
